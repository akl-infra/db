...
// the identity (`as` equals the record's own format) or a `to[as]`
// translation; there is no path from "store" to "read the same format
// back" that touches `lower()` at all. 901-idioms.json is the fixture
// built for exactly this (repeat key + except + explicit rules + chiral
// key + except + adaptive swap + raw rules, on a realistic 30-key layout).
...
const FIXTURE = path.resolve(import.meta.dirname, "..", "..", "formats", "akl", "1", "fixtures", "901-idioms.json");
...
describe("intent survives store + read (LDB-F3)", () => {
  ...
  it("901-idioms.json validates as akl/1", () => {
    const akl1 = getFormat("akl/1")!;
    expect(akl1.validate(payload).ok).toBe(true);
  });
  ...
  it("[LDB-F3] reading it back as its own format (the registry's identity path) keeps adaptive_swaps intact", () => {
    ...
    expect(akl1.validate(readBack).ok).toBe(true);
  });
  ...
  it("[LDB-F3] the registry's own translate() never calls a format's lower() on the identity path", () => {
    // Static check on registry.ts's own source: `translate()` returns
    // `{ payload: rec.payload }` verbatim when `as === rec.format`, before
    // ever touching `source.to[as]` (which is the only place any format's
    // `lower()` gets called, transitively, via toCmini/fromCmini). Reading
