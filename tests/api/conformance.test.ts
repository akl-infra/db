...
// LDB-A4 follow-up: seeded 'revoked' from the start (never goes through the
// admin revoke route, unlike `CONFORMANCE_CLIENT_DELETE_ID` above) -- the
// client-lane sweep's `client_revoked` fixtures (manifest.ts's
// `CLIENT_LANE_CASES`, generated by scripts/gen-client-lane-sweep.mjs) name
// this fixed id directly in their `X-Akl-Client` header.
const CONFORMANCE_CLIENT_REVOKED_ID = "conformance-client-revoked-1";
// [LDB-A11] rogue-trusted-client hardening: two more well-known clients --
// one seeded `active` (mutated to `suspended` by `admin-clients/suspend-
// 200.json`, so it must run AFTER `admin-clients/200.json`'s own listing
// snapshot, same file-order discipline `CONFORMANCE_CLIENT_DELETE_ID`
// already relies on), one seeded `suspended` from the start (mutated to
// `active` by `admin-clients/reactivate-200.json`).
const CONFORMANCE_CLIENT_SUSPEND_ID = "conformance-client-suspend-1";
const CONFORMANCE_CLIENT_REACTIVATE_ID = "conformance-client-reactivate-1";
...
async function seedWriteFixtures(): Promise<void> {
  ...
  await db.batch([
    ...
    // LDB-A4 follow-up: seeded 'revoked' directly (never goes through the
    // admin revoke route) so the client-lane sweep's `client_revoked`
    // fixtures see a client that was ALWAYS revoked -- `verifyClientRequest`
    // reads `status` fresh on every request, never caches it, so this needs
    // no route call to set up.
    db
      .prepare(
        `INSERT OR IGNORE INTO clients (id, name, pubkey, owner_user_id, caps, discord_app_id, status, created_at, revoked_at)
         VALUES (?, ?, ?, ?, ?, NULL, 'revoked', ?, ?)`,
      ...
      .bind(
        CONFORMANCE_CLIENT_REVOKED_ID,
        "conformance-client-revoked",
        vectors.keys[0]!.pubkey_b64url,
        CONFORMANCE_OWNER,
        "act-as-user",
        CONFORMANCE_CLOCK_ISO,
    ...
    db
      ...
      .bind(CONFORMANCE_CLIENT_SUSPEND_ID, "conformance-client-suspend", vectors.keys[0]!.pubkey_b64url, CONFORMANCE_OWNER, "act-as-user", CONFORMANCE_CLOCK_ISO),
    db
      .prepare(
        `INSERT OR IGNORE INTO clients (id, name, pubkey, owner_user_id, caps, discord_app_id, status, created_at, revoked_at)
         VALUES (?, ?, ?, ?, ?, NULL, 'suspended', ?, NULL)`,
      )
      .bind(CONFORMANCE_CLIENT_REACTIVATE_ID, "conformance-client-reactivate", vectors.keys[0]!.pubkey_b64url, CONFORMANCE_OWNER, "act-as-user", CONFORMANCE_CLOCK_ISO),
  ]);
}
...
const ERROR_CODES = {
  ...
  // not just writes). `actor_not_allowed` (403, `act-as-owner-only`'s
  // refusal) is deliberately NOT here -- it isn't a 401 and isn't part of
  // the A-group; it stays pinned only in tests/auth/client.test.ts.
  bad_signature: badSignature().body.error,
  unknown_client: unknownClient().body.error,
  client_revoked: clientRevoked().body.error,
  stale_timestamp: staleTimestamp(999).body.error,
  replay: replay().body.error,
  // [LDB-A11] rogue-trusted-client hardening.
  client_already_revoked: clientAlreadyRevoked().body.error,
};
...
//
// LDB-A4 follow-up: every route reachable through `A` resolves an actor via
// `resolveActor` (09 §2.1), which dispatches to the client lane on
// `X-Akl-Client` alone (10 C1 D9) -- so the client lane's own five 401
// codes are reachable on every one of these routes too, same as the bearer
// lane's `unauthorized`/`token_invalid`/`identity_unavailable`. Fixture
// cases for all five, on every A-group route, live in manifest.ts's
// `CLIENT_LANE_CASES` (generated by scripts/gen-client-lane-sweep.mjs).
const A: RequiredCase[] = [
  ...
  { status: 401, code: ERROR_CODES.bad_signature },
  { status: 401, code: ERROR_CODES.unknown_client },
  { status: 401, code: ERROR_CODES.client_revoked },
  { status: 401, code: ERROR_CODES.stale_timestamp },
  { status: 401, code: ERROR_CODES.replay },
];
...
const REQUIRED: Record<string, RequiredCase[]> = {
  ...
  // lighter treatment, same posture as `POST /v1/admin/dump`/`POST
  // /v1/admin/magic-seed` above (unauthorized + not_admin + the route's
  // own domain outcomes, not the full client-lane `...A` sweep).
  "POST /v1/admin/clients/:id/suspend": [
    { status: 200 },
    { status: 401, code: ERROR_CODES.unauthorized },
    { status: 403, code: ERROR_CODES.not_admin },
    { status: 404, code: ERROR_CODES.not_found },
    { status: 409, code: ERROR_CODES.client_already_revoked },
  ],
  "POST /v1/admin/clients/:id/reactivate": [
    { status: 200 },
    { status: 401, code: ERROR_CODES.unauthorized },
    { status: 403, code: ERROR_CODES.not_admin },
    { status: 404, code: ERROR_CODES.not_found },
  ...
  "POST /v1/admin/clients/:id/revert": [
    { status: 200 },
    { status: 401, code: ERROR_CODES.unauthorized },
    { status: 403, code: ERROR_CODES.not_admin },
    { status: 404, code: ERROR_CODES.not_found },
...
describe("conformance enumeration", () => {
  ...
  // LDB-A4 follow-up: structural, not fixture-content-dependent -- `A`
  // itself carries all five client-lane 401 codes (this would fail the
  // moment someone trims `A` back down), and every REQUIRED route built
  // from `...A` inherits every one of them. The two enumeration tests above
  // ("every (route, required status, code) pair has a conformance case" /
  // "every case names a ... pair that is REQUIRED") already prove a real
  // fixture exists for each -- this test is the one place that says WHY
  // there should be that many: every route in the sweep pins every
  // client-lane 401 code.
  it("[LDB-A4] every route in the sweep pins every client-lane 401 code", () => {
    const CLIENT_LANE_401_CODES = [
      ERROR_CODES.bad_signature,
      ERROR_CODES.unknown_client,
      ERROR_CODES.client_revoked,
      ERROR_CODES.stale_timestamp,
      ERROR_CODES.replay,
    ];
    for (const code of CLIENT_LANE_401_CODES) {
      expect(A, `A is missing client-lane code '${code}'`).toContainEqual({ status: 401, code });
    }
    const missing: string[] = [];
    for (const [route, required] of Object.entries(REQUIRED)) {
      const isAGroupRoute = A.every((a) => required.some((r) => r.status === a.status && r.code === a.code));
      if (!isAGroupRoute) continue;
      for (const code of CLIENT_LANE_401_CODES) {
        if (!required.some((r) => r.status === 401 && r.code === code)) missing.push(`${route}: ${code}`);
      }
    }
    expect(missing, `A-group routes missing a client-lane 401 code:\n${missing.join("\n")}`).toEqual([]);
  });

  // [LDB-P2] the fixture-loop `it(kase.id, ...)` above proves each named
  // case byte-exact, but `kase.id` is a variable, never a string literal,
  // so none of those titles can ever carry a `[LDB-P2]` tag for
  // tests/tools/invariants.test.ts to find -- this one is a real literal
  // title proving the structural half: every write verb LDB-P2 covers
  // (`PUT`/`PATCH`/`DELETE`/`transfer` -- the ones with a prior version to
  // name) has its own pinned `400 if_match_required` REQUIRED row, which
  // the enumeration tests above already prove has a real fixture case.
  it("[LDB-P2] every write verb needing a prior version pins if_match_required", () => {
    const routesNeedingIfMatch = [
      "PUT /v1/layouts/:ref",
      "PATCH /v1/layouts/:ref",
      "DELETE /v1/layouts/:ref",
      "POST /v1/layouts/:ref/transfer",
    ];
    for (const route of routesNeedingIfMatch) {
      const required = REQUIRED[route] ?? [];
      expect(required, `${route} missing if_match_required`).toContainEqual({ status: 400, code: ERROR_CODES.if_match_required });
    }
  });
});
