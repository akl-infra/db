...
| SITE-3 | A non-safe `/api/v1/*` request without `X-Requested-With: akldb`, or with a cross-site `Sec-Fetch-Site`, is `403 csrf` and never reaches the DB; a safe (GET) request needs neither header. | `tests/server/proxy.test.ts` |
