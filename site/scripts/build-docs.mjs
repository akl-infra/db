...
const ENDPOINT_PURPOSES = {
  ...
  "POST /v1/admin/import/unstall": "Lift a stalled cmini import deliberately; the next tick re-evaluates from scratch and re-stalls if upstream is still mass-deleted.",
  "POST /v1/admin/import/restore-deleted": "Bulk-restore layouts tombstoned by the upstream import since a timestamp (bounded, idempotent, dry-run first).",
  ...
  "POST /v1/admin/clients/:id/suspend": "Suspend a trusted client immediately -- unlike revoke, reversible with reactivate.",
  "POST /v1/admin/clients/:id/reactivate": "Restore a suspended client's access, with a fresh destructive-write budget window.",
  "POST /v1/admin/clients/:id/revert": "Undo a client's destructive writes since a timestamp, newest first, dry-run first.",
