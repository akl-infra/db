// Regression: every admin tab's list load (Bans, Admins, Link queue) must
// show the DB's real error `message` when the GET itself answers
// `ok:false` (e.g. a real `403 not_admin` from the live DB when this
// dev-mock admin carries no real bearer token -- caught during this
// slice's own Chrome QA against production akl-db: `createAsync`'s
...
// rows -- `lib/apiError.ts`'s `loadErrorMessage` is the one place all
// three tabs (BansTab/AdminsTab/LinkQueueTab) read this from, so it's
// tested here directly rather than through three separate component
// mounts.
