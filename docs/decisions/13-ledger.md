...
**Post-seed checks, two follow-ups.** (1) `scripts/verify_magic_migration.py` reports a systematic one-row mismatch on ~38 layouts: the site's `magicRulesFlatCompile` emits a word-start row (space-prefixed input, the I-153/I-154 word-boundary rules) that `db/formats/akl/1`'s `lower()` does not — only `?as=cmini/1` readers see it; the bot's stats use the site's compiler. Align `lower()` with the site's word-boundary rows (a db format change with goldens), then the verify goes green apart from the 14 site-only layouts. (2) The seed's PUTs bumped `modified_at` on 67 records, so the daily diff flags them until the next non-quiet import tick re-adopts upstream's; magic-only writes now keep `modified_at` (write.ts, 2026-09-10 05:xxZ) so it cannot recur.
