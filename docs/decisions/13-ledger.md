...
**Post-seed checks, two follow-ups.** (1) **DONE**, `ldb-lower` branch (LDB-F14): `scripts/verify_magic_migration.py` reported a systematic one-row mismatch on ~38 layouts: the site's `magicRulesFlatCompile` emits a word-start row (space-prefixed input, the I-153 word-boundary rule) that `db/formats/akl/1`'s `lower()` did not — only `?as=cmini/1` readers saw it; the bot's stats use the site's compiler. `lower()`'s `computeRows` now emits `{inputs: ' '+key, output: ' '+default}` for every magic key whose default is a literal character, mirroring the site's condition exactly (`repeat_previous` gets none; `except` does NOT suppress it, matching the site, which has no such list for this row; an explicit `magic_keys[].rules[]` entry for `after: ' '` still replaces it); `liftRules` absorbs the row back into the idiom with no leftover, and `findCollision` treats it like any other scaffold row (a raw rule on `' '+key` collides, with no `except` hint since one wouldn't fix it). New fixture `formats/akl/1/fixtures/903-word-start.json` + regenerated goldens, new `tests/formats/magic-word-start.test.ts` (LDB-F14). Not yet deployed (this branch never deploys) — the preview still serves the old lowering, so `verify_magic_migration.py` needs a re-run against `akl-db-preview` AFTER this lands and a deploy goes out; it still reports the mismatch until then. (2) The seed's PUTs bumped `modified_at` on 67 records, so the daily diff flags them until the next non-quiet import tick re-adopts upstream's; magic-only writes now keep `modified_at` (write.ts, 2026-09-10 05:xxZ) so it cannot recur.
