...
| thing | where | state |
|---|---|---|
| `akl-db` Worker (production) | Cloudflare account **akl** (community, Workers Paid), `https://akl-db.akl-58a.workers.dev` | deployed by hand from the branch (last: 1adf8ac6 as v3bf67ca1, 2026-09-10 03:10Z — M1: the import ignores cmini's magic, weak-ETag 304s). D1 `akl-db` (migrations 0001–0004), R2 `akl-db-dumps`. If-Match required on every write (LDB-P2). Workers Logs on. Admin kicks: `POST /v1/admin/{import,diff,nightly}/tick`; `POST /v1/admin/import/strip-cmini-magic` run once on 2026-09-10 (15 records; 0 on the second call) — no production record carries cmini magic any more. |
...
| `spark` bot | saltorbit's Fly account, app `spark-bot`, iad, **1 GB** (256 MB thrashed, 512 MB alerted at rest), volume `botdata` at `/data` | deployed by hand from the branch (last: b2c46a0e, 2026-09-10 03:0xZ — fonts in the image (LDB-B22), feed diagnostics (B23), akl.gg's magic rules everywhere (B24), masked `[(akl.gg link)]` on every single-layout reply on the site id (B25), 6× images, option-D finger colors on the text grids (B29); verified live through the harness). Prefixes `!spark !sp !aklgg !ag`. Points at production. Verify-then-serve reads, fresh `If-Match` writes, SSE feed, memory watchdog + failure DMs to saltorbit, lazy per-corpus harvest (idle ≈ 190 MB). `TEST_BOT_IDS` secret = spark-tester, so the e2e harness drives production. |
