...
| Site PREVIEW with the write path | same project, Pages preview alias `db` → `https://db.cmini-web.pages.dev` | LIVE 19:30Z from run 34393036960 (`gh workflow run build.yml --ref worktree-layout-db`; the first run failed on a `${{ vars }}` expression inside the cmini-scrape action's description — fixed f5cba2f6). Root 200, `/api/db/me` 401 signed-out, login redirects to Discord (the Spark application's client id). Preview env: `DB_BASE_URL` = the preview DB, `TOKEN_KEY` set, `DEPLOY_GIT_REF`=worktree-layout-db, `DEPLOY_PAGES_BRANCH`=db, Discord secrets from the magic era; D1 = the shared `cb-magic` with migration 0008 applied 19:00Z (additive `discord_tokens` table). Needs saltorbit: add redirect URI `https://db.cmini-web.pages.dev/auth/discord/callback` to the Spark application's OAuth2 settings. Re-deploy = re-dispatch build.yml on the branch. |
