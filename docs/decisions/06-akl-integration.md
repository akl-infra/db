...
| LDB-S4 | After the magic migration, no layout's served rules differ from before it (the site's compile of the static `magic_rules.json` == the DB's lowering, as sets of `(inputs, output)`); the migration is idempotent. | `scripts/tests/test_migrate_magic_rules.py`, `web/tests/backend/magic-migration-verify.test.js`, the one-shot `verify_magic_migration.py` *Amended 2026-09-11: `migrate_magic_rules_to_db.py` applies `magic_collision` hints until the payload validates (validate() reports one collision per call), so a rule set with several scaffold-vs-idiom collisions resolves every one the way akl.gg's compile order already does (the swap wins); only a hintless collision is refused. **Superseded the same day:** spark/1 now resolves idiom overlaps itself (LDB-F4 amended) and accepted off-layout keys (LDB-F22, until both validators began refusing them together on 2026-09-13, saltorbit/aklgg#322), both as akl.gg does, so the migration writes every rule set as published; the hint loop remains for a raw-row collision (`HintLoop` tests, stubbed validator).* |
