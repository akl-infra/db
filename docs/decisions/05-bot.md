...
| LDB-B46 | The image card's text is WCAG AA (>= 4.5:1) on the card background for every theme -- footer, stats sub-labels, compare deltas -- through the shared renderer's own defaults (`@akl/core/copyimage`), which akl.gg's copy-as-image paints too (site I-250); the bot adds no override. | `bot/tests/render/image.test.ts` |
| LDB-B47 | Every text block that prints stats under a corpus line tags it `· SG On` / `· SG Off` (the image footer's signed words), from whether the numbers were computed with a space thumb; the same flag drives the NoTh/Thumb rows. | `bot/tests/render/grid.test.ts`, `bot/tests/commands/read.test.ts` |
