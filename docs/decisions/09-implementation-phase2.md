...
| `appendLike` | pre-checks `likes`, then `UPDATE layouts SET like_count = ?` (absolute) | two users liking at once lost one count; the same user twice at once threw on the `likes` PK. **Fixed on the branch before T1**: `like_count` is recomputed from the `likes` table inside the batch and the PK conflict is the idempotent no-op (`races.test.ts`) |
