...
3. **W5's migration against production** (`scripts/migrate_magic_rules_to_db.py --base-url <prod DB>`; report attached to the flip's issue), then `verify_magic_migration.py` exit 0; then the code that (a) points `build_magic_rules.py` at the DB's `?as=akl/1` records for `magic_rules.json` — **landed** (`ldb-m3-sync`, `17 §4 M3`: `--source db --base-url`, cursor-paged `has_magic=true` + per-id detail, the seed folded in for site-only layouts; `build.yml` branches on `vars.DB_BASE_URL`), (b) **amended 2026-09-10 (saltorbit: the PUT does NOT become a 410; akl.gg keeps its editor)** — `functions/api/magic-rules/[id].js`'s GET reads the record's `payload.magic` and its PUT writes `PATCH /v1/layouts/{id} {magic}` through the proxy when the Pages env var `DB_BASE_URL` is set; `index.js` answers an event-feed overlay — **landed** (`ldb-m3-live`, `17 §4 M3`'s live half, LDB-S7..S11), (c) **amended**: `magic-rules-sync.yml` is not disabled — a DB-mode publish still needs its per-layout stat patch and nothing consumes `db_site_write` yet, so its rules-fetch step reads the DB when `vars.DB_BASE_URL` is set; `magic-rules-backup.yml` parks on the same variable (the DB's dump is the backup) — **landed** (`ldb-m3-live`), (d) drops `fetch_d1_rules.py`'s D1 read behind `--source db` — the `--source db` half **landed** with (c); the D1 branch is deleted one release after the flip, together with the `magic_rules` table. Rollback for (a)–(d): clear the two `DB_BASE_URL`s; D1 still holds the rows.
