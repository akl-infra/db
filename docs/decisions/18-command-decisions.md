...
- **C14 `theme` for images** (saltorbit, 2026-09-10: "can we add a theme command that lets you switch between default and colorblind mode for images"). **SHIPPED 2026-09-10 (LDB-B45)**: `!theme [default|colorblind]`, per user in `prefs.ts` beside corpus/spacegrams; `!image` paints with akl.gg's own `activeFingerColors(theme)`; text grids keep their three ANSI colors. Same thread, xsznix: "that footer text is definitely not WCAG"; saltorbit: "probably that should just be the new default" / "if you go colorblind mode, it should make the footer more visible" → **the footer is WCAG AA for every theme (LDB-B46)**: watermark 1.75:1 and LH/RH separator 1.40:1 raised to `IMG_COLORS.dim` (5.40:1). **Open (not built):** xsznix's "color schemes, maybe we could get a textmate color schemes collection from somewhere to import" — a bigger idea (card chrome + key palettes from imported schemes); needs saltorbit's call on scope and whether the site gets the same themes.
