...
- **C13 `spacegrams` as a user setting** (saltorbit, 04:1xZ): `spacegrams on|off` beside `corpus` (`prefs.ts`, per user, on the Fly volume); every stat verb and `!image` compute with it (the harvest's spacegrams variant of the cell / the engine's `context.space`), and the card's footer says which (`· spacegrams` / `· no spacegrams`; the static "no spacegrams" note shipped 2026-09-10 is the interim).
