...
| H24 | **Likes are always tied to the users who liked** (saltorbit 2026-09-13: "likes should always be tied to the users who liked it, not be just an opaque number you can set"). `like_count` is derived from `likes` rows (layout_id, user_id, at, via) and nothing else; no route, event, import or admin action may set or adjust it directly. The cmini import brings likes over as per-user rows or not at all (never a bare count). |
