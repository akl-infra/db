...
| id | slice | brief | status |
|---|---|---|---|
| B1 | **cell store + row tables + rank** | content-hash-keyed cell store persisted on `/data`; eager compute on rev bump (wasm, 39 cells); per-(corpus, space) row tables; `rank` sorts at once, footer stand-in for pending rows; delete `provenance.ts` + publish tracker + adoption resets; LDB-B rows. | todo |
...
1. Lead creates the slice worktree from the integration branch (script in scratchpad): `git worktree add .claude/worktrees/ldb-<slice> -b ldb-<slice> ldb-arch-review`.
2. ONE agent at a time. Brief = the slice row above + the relevant audit section + "state invariants, add tests, run the suite, commit on your branch, do not push, do not deploy, report branch + sha + test counts".
