...
| B6 | **boot never waits on the sweep** (running: impl-B6) | INCIDENT 2026-09-12 17:55–18:02Z: the wave-2 image never reached `client.login()` (first boot: empty cell store → `warmBoot` enqueued 4,175 layouts; sweep held the engine lock; boot's `warmCorpus` waited behind it; resident-table budget 8 vs 39 tables/layout thrashed loads; no per-phase boot logs; `/health` started after boot so Fly's check never passed). Rolled back to v32 with `flyctl machine update --image`. Fix: health server first with `phase: booting`; per-phase boot logs; sweep starts only after `clientReady` + warm-up; two-lane engine lock (commands/boot preempt the sweep) + idle gap; bounded `warmCorpus`; table-major sweep order (default corpus first); publisher CLI gated the same way. | running |
...
- 2026-09-12 · B5 landed (eeae69346). B6 rebasing over V0.
