...
16. **D16 · Operability addenda (saltorbit, 2026-09-09): a memory watchdog, DB-call robustness, and a bot-authored test harness allowlist.** Three small, independent additions landed alongside D15's fresh-read/fresh-write work, each ledgered here rather than reopening D1-D15. **(a)** `watchdog.ts` (LDB-B15): with `ALERT_USER_ID` set, a 60 s timer DMs saltorbit once when `process.memoryUsage().rss` crosses `ALERT_MEMORY_PCT` (default 80) of the cgroup memory limit (`/sys/fs/cgroup/memory.max`, falling back to `os.totalmem()` off-cgroup) and once more on recovery (10 points of hysteresis, so oscillating near the line doesn't repeat the DM); separately, `<dataDir>/spark.pid-marker` — written every boot, removed only by a clean SIGTERM/SIGINT — still being present at the NEXT boot means the previous run died some other way (OOM kill, crash, a forced `fly machine restart`) and DMs once with that marker's timestamp. **(b)** A production finding (`!sp swap! dsfs ou` hung ~1 minute, answered the DB's raw 500 body, and logged nothing) drove LDB-B16: `client/http.ts`'s `dbFetch` now aborts after `timeoutMs` (20 s default) and turns that into an ordinary `{status: 0, error: 'timeout'}` result rather than a hang or a throw, so every write verb's existing generic-error fallback already reports `copy.ts`'s `dbTimedOut()` with no per-verb change; every non-2xx response on a write (never a read) is `console.error`'d with the method, path, the DB's own `error`/`message` and the elapsed ms, never the signed headers; `handleMessage` separately logs any command (`verb` + elapsed ms) that takes over 5 s to answer. **(c)** The DB's own new `requireIfMatch` (landed the same day, `core/ifmatch.ts`, `400 if_match_required` on a bodied write with no `If-Match`) should be unreachable from this bot given D15's "always sends If-Match" guarantee — `writeWithFreshRecord` logs a distinct `BUG:` line if it's ever seen anyway, on top of the ordinary generic-error answer (still no retry). **(d)** `TEST_BOT_IDS` (LDB-B17, empty in production, never in `fly.toml`): `handleMessage`'s bot-author check now allowlists these ids, so a "spark-tester" bot in saltorbit's test server can drive a second spark instance (prefix `!spt`, pointed at the preview DB) for an end-to-end harness — a bot message from an allowlisted id dispatches exactly like a real user's, actor = that bot's own id. Copy strings (`dbTimedOut`, `memoryAlertHigh`, `memoryAlertRecovered`, `restartedAfterNonCleanExit`) sign-off pending like every other `copy.ts` addition. `bot/tests/watchdog.test.ts`, the `client/http.test.ts`/`commands/write.test.ts` additions, and `bot/tests/main.test.ts` are the tests.

17. **D17 · Two live e2e bugs, fixed (2026-09-09), from the first real run against the `!spt` preview instance.** **(a)** `!add`'s grid: `router.ts`'s `parseMessage` whitespace-tokenized the WHOLE message, so a fenced grid in the same message as the command (cmini's own `!add`/`!setfingermap` shape) leaked its fence markers and rows into `args` -- `add.ts` builds the name from `args.join('-')` and refused with "names cannot contain `` ` ``". cmini's own `util/parser.py:get_layout` splits on the first ` ``` ` BEFORE tokenizing (`tokens[0].split()`); `parseMessage` now does the same, tokenizing only the text before the first fence. `main.ts`'s `extractCodeBlock` (unchanged in scope, still pulls the FIRST fenced block only) additionally strips a leading language-tag line (Discord's own convention, e.g. ` ```text `) so a habit-typed tag never becomes a spurious first grid row. **(b)** `!authors` never answered within the harness's 30 s wait on the 4 176-record preview cache -- not a slow verb (its own body is a single O(n log n) sort, confirmed via `spt.log`'s actual stack trace: `DiscordAPIError[50035] Invalid Form Body`, `content[BASE_TYPE_MAX_LENGTH]: Must be 2000 or fewer in length`), but a reply too long for Discord's own hard limit, rejected instantly and only logged by `main.ts`'s outer `.catch` -- the bot never replied at all, which is what read as a "timeout" from outside. Fixed with a byte-budget truncation (LDB-B19): names are added until the next one would cross 2 000 chars, then `...and N more` replaces the rest -- unlike `list.ts`'s existing fixed 100-item cap (which bounds count, not length; `!authors` has no per-owner filter keeping its count small, and an uncached author's name can be as long as their raw Discord id, D14). `bot/tests/router.test.ts`'s new fenced-block group and `bot/tests/commands/read.test.ts`'s new `[LDB-B19]` group are the tests.
18. **D18 · Production images were blank (2026-09-09, saltorbit: "images were broken through that" -- the first `!sp image` runs against the Fly instance).** The Dockerfile's final stage copied `dist/` but never `bot/fonts/`; `node:24-slim` ships no system fonts, so `@napi-rs/canvas` drew every card's text with an unresolved alias -- a well-formed PNG with no glyphs. Nothing caught it: `GlobalFonts.registerFromPath` returns `null` for a missing file and `ensureFontsRegistered` ignored the return, and the laptop `!spt` instance (where the e2e harness ran) rendered correctly because macOS resolved `"Helvetica Neue"`/`"Menlo"` from its own font set, so the transcript's `image graphite` attachment looked fine. Fix (LDB-B22): `COPY bot/fonts ./fonts` in the runtime stage; `ensureFontsRegistered` verifies each of `FONT_FILES` exists and registers, throwing a named `FontsMissingError` otherwise (never memoizing a failure); `bootRuntime` runs it at boot and, on failure, logs and reports `bug` to the operator DM but keeps booting (a crash loop over a font would take every text verb down with it) -- `!image` re-throws on use, so the verb answers an error instead of attaching a blank card. `bot/tests/tools/dockerfile.test.ts` (new) pins the copy beside `./dist` under `/app`; `bot/tests/render/image.test.ts`'s `[LDB-B22]` group pins the throw. Lesson for the harness: an attachment's *presence* is not a rendering check -- a run against the laptop instance cannot see a fonts problem the Docker image has; only a run against the Fly instance (`E2E_ALLOW_PRODUCTION=1`, 13-ledger §3) exercises the real artifact.
