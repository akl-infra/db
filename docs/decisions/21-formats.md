...
New or changed; ids are placeholders until the slice picks the next free number in `db/INVARIANTS.md` / `bot/INVARIANTS.md` / `design/INVARIANTS.md`. "The write model" below is one `fast-check` model shared by MF-1, 2, 3, 6 and 12: random sequences of every write kind in §2.2 (user and import, both scopes, over spark and the test-only second lineage), including concurrent pairs.

| id | invariant | enforced by |
|---|---|---|
| MF-1 | **Format independence.** A write to format A never changes format B's payload, rev, created_at or modified_at; a layout-level write changes no format row. | the write model: every row outside the write's scope is byte-equal before and after each step |
| MF-2 | **Rev partition.** Every rev-bumping event has exactly one scope: `format` NULL bumps `layout_rev` by 1, otherwise it bumps that format's `rev` by 1. A write that changes both scopes (a create, an import) appends one event per scope. Per layout, `layout_revs.n` is gapless from 1; per `(layout, lineage)`, `rev` is gapless from 1; per layout, `layout_rev` equals its count of layout-level events. | the write model, plus a SQL check in the drill |
| MF-3 | **Replay equivalence.** Folding a layout's events in seq order, with payloads from `layout_revs`, reproduces its `layouts` row and every `layout_formats` row exactly. | the write model (extends today's `foldRecord` test) |
| MF-4 | **Explicit format.** Every route that returns or changes a payload answers `400 format_required` without one; no route has a default. | generated matrix, enumerated from the router table (`LDB-G10` already walks it): one row per route × {format missing, stored and present, stored but absent on this layout, output (mana2/1), unregistered}, each with its expected status and code |
| MF-5 | **At least one format.** Every layout has at least one `layout_formats` row: creation takes one and nothing removes one this round (§2.4). | the write model's per-step check, plus a SQL check in the drill |
| MF-6 | **Concurrency per scope, serialization per layout.** Two writers on one format with the same If-Match: exactly one lands, the other gets `409 stale` with that format's current rev. Writers on different scopes of one layout both land. A format write racing a transfer or delete that lands first is refused by the new owner/deletion state, never landing on a tombstone or for a former owner. | race tests (extend `db/tests/events/races`), including forced PK collisions to exercise the retry path |
| MF-7 | **Derived formats are never stored.** A `?format=mana2/1` read never writes, and its response names `derived_from`. A read of a stored format is never derived. | matrix rows |
| MF-8 | **The bot only reads spark.** Every layoutdb GET the bot issues names `format=spark/1`. An event for another lineage changes nothing in the bot's cache except that layout's format names. A layout with no spark row answers "has no spark/1 format", never "not found", and its `404 format_absent` never stalls the drain. | bot test over a feed with mixed-lineage events and spark-less layouts; a matrix row per lookup verb |
| MF-9 | **fromCmini is exact where spark has a place.** For every cmini layout, the multiset of (char, row, col, finger) equals spark/1's `keys` entries, cmini's board word maps to spark's `board` by the fixed table, and the fields dropped are exactly `tag`, `blame`, `combos`, `link`. | property test over `upstream-100` with a test-local projection (replaces the `toCmini` round trip deleted by D5) |
| MF-10 | **Unambiguous derivation.** Each `role: "output"` format is reachable from exactly one `role: "stored"` lineage. | registry test over `list()`; the stub lineage's deliberately broken variant proves it catches a second source |
| MF-11 | **Scoped If-Match.** A write accepts only its own scope's token (or `*`); a bare number or another scope's token is `400 bad_request`, before any read. Every write response's `ETag` is its scope's token. | generated matrix: every write route × {absent, bare number, own scope, other scope, `*`} |
| MF-12 | **Follow scope.** A user write forks a following layout iff it is layout-level or to lineage `spark`; a write to another lineage never changes `upstream`; the importer never writes a forked layout. | the write model (restates LDB-I14 and LDB-P11) |
| MF-13 | **Dump floor.** Every table in a dump is at or past the dump's `meta.seq`, so booting from the dump and draining the feed from `meta.seq` equals the live state (LDB-B10 with two tables). | a test that lands writes between the dump's reads, then checks boot + drain against live |
| LDB-F10 | retired (D10). | — |
