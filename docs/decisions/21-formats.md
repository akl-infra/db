...
| D8 | **No data migration.** layoutdb is wiped and rebuilt from cmini and akl.gg after the schema change (standing rule: layoutdb is disposable). **Amended 2026-09-14 (saltorbit: "we can't lose data anymore - people may start making edits"):** the standing rule is retired. People edit records now, through the bot and the site; akldb is never wiped again, and every migration from here on is additive -- no `DROP TABLE`, no `DELETE FROM`, never recreating `events`. Rewriting stored rows in place (`UPDATE ... SET payload_json = ...`, as `0016_no_board.sql`/`0017_magic_emit.sql` already did) is still allowed -- it changes what a row holds, not whether it exists. Enforced going forward by `tests/tools/migrations-additive.test.ts` (LDB-G15), which scans every migration for the retired statements against a closed, checked allowlist of the pre-2026-09-14 files that legitimately contained them. |
...
- Verified live: spark-tester `!sp view graphite` answers; `like`, repeat `like` ("You've already liked this layout"), `unlike`, repeat `unlike` ("You've already unliked this layout") all behave per D13 at about 2.3 s round trip including Discord. e2e `--dry-run` against prod: layoutdb, the headless bot (boot 1.1 s, `view` 798 ms, `sfbs` 176 ms) and the site pass; D4 (bucket vs layoutdb) failed until the fresh stats service's first publish replaced the pre-wipe overlay's ids; that landed at 04:11Z (overlay 20260912T041030Z-7657d91f, pointer db_seq 23919), and the re-run is ALL PASS, including D9 (bot vs bucket numbers) and D11 (formats map). D12 is skipped by design: production has no spark-less layout.
