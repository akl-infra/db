...
| D11 | **LDB-F6 (a published format never changes) is suspended until layoutdb's first outside adopter.** spark/1 is edited in place this round (D5, D10); a spark/2 now would be ceremony, since nobody else writes spark/1 and the database is rebuilt anyway. The F6 test gets an explicit, documented start point instead of comparing against `origin/main`. |
